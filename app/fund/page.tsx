...
type DbPosition = {
  ...
  usddd_mint_tx_hash?: string | null;
  usddd_transfer_tx_hash?: string | null;
...
export default function FundNetworkPage() {
  ...
  function computeAccruedDisplay(p: any): number | null {
    ...
    const start = p?.usddd_accrual_started_at ? Date.parse(String(p.usddd_accrual_started_at)) : NaN;
                      ...
                      return (
                        <tr key={p.id} className="border-b border-slate-800/40 align-top">
                          ...
                          <td className="py-2 pr-4 text-right">
                            {Number(p.usddd_allocated ?? 0) ? (
                              <div className="text-[11px]">
                                <div>{fmtNum(Number(p.usddd_allocated))}</div>
                                {p.usddd_transfer_tx_hash ? (
                                  <div className="text-slate-500">
                                    <TxLink hash={p.usddd_transfer_tx_hash} />
                                  </div>
                                ) : null}
                              </div>
                            ) : (
                          ...
                          <td className="py-2 pr-4 text-right">
                            {(() => {
                              ...
                              return v == null || v <= 0 ? "â€”" : fmtDec(v, 6);
                            })()}
                          </td>
